...
TABLES = {
    ...
    "api_keys": "api_hub.api_keys",
    "webhooks": "api_hub.webhooks",
    "integration_runs": "api_hub.integration_runs",
}
SOFT_DELETABLE = frozenset({"api_clients", "api_keys", "webhooks", "integration_runs"})


class ApiHubPostgresStore:
    """Production API Hub adapter backed by typed PostgreSQL relations and central audit/outbox."""

    module = "api_hub"
    backend = "postgres_api_hub_typed_store"
    ...
    def _insert(
        ...
        if resource_type == "api_clients":
            return connection.execute(
                """INSERT INTO api_hub.api_clients
                   (id, user_id, company_id, client_name, client_id_hash, secret_reference, scopes,
        ...
        if resource_type == "api_keys":
            return connection.execute(
                """INSERT INTO api_hub.api_keys
                   (id, user_id, company_id, client_id, key_name, key_hash, key_hint, scopes,
        ...
        if resource_type == "webhooks":
            return connection.execute(
                """INSERT INTO api_hub.webhooks
                   (id, user_id, company_id, target_url, event_patterns, signing_secret_reference,
                    status, metadata, created_by, updated_by, idempotency_key)
        ...
        if resource_type == "integration_runs":
            return connection.execute(
                """INSERT INTO api_hub.integration_runs
                   (id, user_id, company_id, integration_type, provider_name, log_summary,
                    status, metadata, created_by, updated_by, idempotency_key)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING *""",
                (
    ...
    def _update(
        ...
        if resource_type == "api_keys":
            return connection.execute(
                """UPDATE api_hub.api_keys SET key_name = %s, status = %s, metadata = %s,
                   updated_by = %s, updated_at = NOW() WHERE id = %s RETURNING *""",
                (payload["key_name"], status, metadata, actor, resource_id),
            ).fetchone()
        if resource_type == "webhooks":
            return connection.execute(
        ...
        if resource_type == "integration_runs":
            return connection.execute(
                """UPDATE api_hub.integration_runs SET log_summary = %s, status = %s,
                   metadata = %s, updated_by = %s, updated_at = NOW() WHERE id = %s RETURNING *""",
                (payload.get("log_summary"), status, metadata, actor, resource_id),
            ).fetchone()
        raise ValueError(f"Recurso API Hub imutavel ou desconhecido: {resource_type}")
    ...
    def _audit(
        ...
        evidence = connection.execute(
            """INSERT INTO audit.logs
               ...
               VALUES (%s, %s, %s, %s, 'api_hub', %s, %s, %s, %s, %s) RETURNING *""",
