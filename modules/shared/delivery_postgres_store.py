...
class DeliveryPostgresStore:
    """Production Delivery adapter backed by typed PostgreSQL relations and central audit/outbox."""

    module = "delivery"
    backend = "postgres_delivery_typed_store"
    ...
    def create(
        ...
        try:
            with self.transaction() as connection:
                ...
                if item is None:
                    raise RuntimeError("PostgreSQL nao retornou recurso Delivery criado.")
    ...
    def _insert(
        ...
        if resource_type == "delivery_requests":
            return connection.execute(
                """INSERT INTO delivery.delivery_requests
                   (id, user_id, assigned_rider_user_id, escrow_id, service_type, origin, destination,
                    distance_km, weight_kg, volume_m3, quoted_brl, insurance_required, status,
                    metadata, created_by, updated_by, idempotency_key)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING *""",
                (
                    resource_id, user_id, payload.get("assigned_rider_user_id"), payload.get("escrow_id"),
                    payload["service_type"], Jsonb(payload["origin"]), Jsonb(payload["destination"]),
                    payload.get("distance_km"), payload.get("weight_kg"), payload.get("volume_m3"),
                    payload.get("quoted_brl"), payload.get("insurance_required", False),
                    status, metadata, actor, actor, idempotency_key,
                ),
            ).fetchone()
        if resource_type == "rider_profiles":
            return connection.execute(
                """INSERT INTO delivery.rider_profiles
                   (id, user_id, wallet_id, cnh_number_hash, cnh_category, status, metadata, created_by, updated_by, idempotency_key)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING *""",
                (
                    resource_id, user_id, payload["wallet_id"], payload.get("cnh_number_hash"),
                    payload.get("cnh_category"), status, metadata, actor, actor, idempotency_key,
                ),
            ).fetchone()
        if resource_type == "vehicles":
            return connection.execute(
                """INSERT INTO delivery.vehicles
                   (id, user_id, rider_profile_id, type, license_plate, status, metadata, created_by, updated_by, idempotency_key)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING *""",
                (
                    resource_id, user_id, payload["rider_profile_id"], payload["type"],
    ...
    def update(
        ...
        with self.transaction() as connection:
            ...
            if updated is None:
                raise RuntimeError("PostgreSQL nao retornou recurso Delivery atualizado.")
    ...
    def _update(
        ...
        if resource_type == "delivery_requests":
            return connection.execute(
                """UPDATE delivery.delivery_requests SET assigned_rider_user_id = %s, status = %s,
                   metadata = %s, updated_by = %s, updated_at = NOW() WHERE id = %s RETURNING *""",
                (payload.get("assigned_rider_user_id"), status, metadata, actor, resource_id),
            ).fetchone()
        if resource_type == "rider_profiles":
            return connection.execute(
                """UPDATE delivery.rider_profiles SET status = %s, metadata = %s, updated_by = %s, updated_at = NOW()
                   WHERE id = %s RETURNING *""",
                (status, metadata, actor, resource_id),
            ).fetchone()
        if resource_type == "vehicles":
            return connection.execute(
                """UPDATE delivery.vehicles SET status = %s, metadata = %s, updated_by = %s, updated_at = NOW()
                   WHERE id = %s RETURNING *""",
                (status, metadata, actor, resource_id),
            ).fetchone()
    ...
    def _audit(
        ...
        evidence = connection.execute(
            """INSERT INTO audit.logs
               ...
               VALUES (%s, %s, %s, %s, 'delivery', %s, %s, %s, %s, %s) RETURNING *""",
    ...
    def audit_log(self) -> list[dict[str, Any]]:
        return [dict(row) for row in self.connection.execute(
            "SELECT * FROM audit.logs WHERE module = 'delivery' ORDER BY created_at DESC"
        ).fetchall()]

    def outbox(self) -> list[dict[str, Any]]:
        return [dict(row) for row in self.connection.execute(
            "SELECT * FROM audit.domain_events WHERE routing_key LIKE 'delivery.%' ORDER BY created_at DESC"
        ).fetchall()]
    ...
    def metrics(self) -> tuple[int, int, int]:
        ...
        audits = self.connection.execute("SELECT COUNT(*) AS count FROM audit.logs WHERE module = 'delivery'").fetchone()["count"]
        events = self.connection.execute(
            "SELECT COUNT(*) AS count FROM audit.domain_events WHERE routing_key LIKE 'delivery.%'"
        ).fetchone()["count"]
