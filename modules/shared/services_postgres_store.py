...
TABLES = {
    ...
    "service_contracts": "services.service_contracts",
}
SOFT_DELETABLE = frozenset({"providers", "service_contracts"})


class ServicesPostgresStore:
    """Production Services adapter backed by typed PostgreSQL relations and central audit/outbox."""

    module = "services"
    backend = "postgres_services_typed_store"
    ...
    def _resource(self, resource_type: str, row: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        return {
            ...
            "entity_id": str(row.get("provider_user_id")) if row.get("provider_user_id") else None,
            "status": row["status"],
    ...
    def create(
        ...
        try:
            with self.transaction() as connection:
                ...
                if item is None:
                    raise RuntimeError("PostgreSQL nao retornou recurso Services criado.")
    ...
    def _insert(
        ...
        if resource_type == "providers":
            return connection.execute(
                """INSERT INTO services.providers
                   (id, user_id, category, status, metadata, created_by, updated_by, idempotency_key)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING *""",
                (resource_id, user_id, payload["category"], status, metadata, actor, actor, idempotency_key),
            ).fetchone()
        ...
        raise ValueError(f"Recurso Services desconhecido: {resource_type}")
    ...
    def update(
        ...
        with self.transaction() as connection:
            ...
            if updated is None:
                raise RuntimeError("PostgreSQL nao retornou recurso Services atualizado.")
    ...
    def _update(
        ...
        if resource_type == "providers":
            return connection.execute(
                """UPDATE services.providers SET category = %s, status = %s, metadata = %s, updated_by = %s, updated_at = NOW()
                   WHERE id = %s RETURNING *""",
                (payload["category"], status, metadata, actor, resource_id),
            ).fetchone()
        if resource_type == "service_contracts":
            return connection.execute(
                """UPDATE services.service_contracts SET contracted_price_brl = %s, status = %s, metadata = %s, updated_by = %s, updated_at = NOW()
                   WHERE id = %s RETURNING *""",
                (payload.get("contracted_price_brl"), status, metadata, actor, resource_id),
            ).fetchone()
        raise ValueError(f"Recurso Services imutavel ou desconhecido: {resource_type}")
    ...
    def _audit(
        ...
        evidence = connection.execute(
            """INSERT INTO audit.logs
               ...
               VALUES (%s, %s, %s, %s, 'services', %s, %s, %s, %s, %s) RETURNING *""",
    ...
    def audit_log(self) -> list[dict[str, Any]]:
        return [dict(row) for row in self.connection.execute(
            "SELECT * FROM audit.logs WHERE module = 'services' ORDER BY created_at DESC"
        ).fetchall()]

    def outbox(self) -> list[dict[str, Any]]:
        return [dict(row) for row in self.connection.execute(
            "SELECT * FROM audit.domain_events WHERE routing_key LIKE 'services.%' ORDER BY created_at DESC"
        ).fetchall()]
    ...
    def metrics(self) -> tuple[int, int, int]:
        ...
        audits = self.connection.execute("SELECT COUNT(*) AS count FROM audit.logs WHERE module = 'services'").fetchone()["count"]
        events = self.connection.execute(
            "SELECT COUNT(*) AS count FROM audit.domain_events WHERE routing_key LIKE 'services.%'"
        ).fetchone()["count"]
